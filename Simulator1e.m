...
transmitedPackets = zeros(1,3); % para guardar o numero de pacotes transmitidos para cada tamanho de pacote 
TRANSMITTEDBYTES= 0;   % Sum of the Bytes of transmitted packets
DELAYS= 0;             % Sum of the delays of transmitted packets
delays= zeros(1, 3); % para guardar os atrasos dos 3 tipos de pacotes
...
while TRANSMITTEDPACKETS<P               % Stopping criterium
    ...
    packetSIZE =EventList(1,3);                %obter o tamanho do pacote
    ...
    switch Event
        ...
        case DEPARTURE                     % If first event is a DEPARTURE
            ...
            %caso o pacote tenha 64 Bytes
            if(packetSIZE == 64)
                delays(1) = delays(1)+(Clock - ArrivalInstant);
                transmitedPackets(1) = transmitedPackets(1)+ 1;
            elseif (packetSIZE == 110)    %caso o pacote tenha 110 Bytes
                delays(2) = delays(2)+(Clock - ArrivalInstant);
                transmitedPackets(2) = transmitedPackets(2)+ 1;
            elseif (packetSIZE == 1518)   %caso o pacote tenha 1518 Bytes
                delays(3) = delays(3)+(Clock - ArrivalInstant);
                transmitedPackets(3) = transmitedPackets(3)+ 1;
            else
...
apd64 = 100*delays(1)/transmitedPackets(1);
apd110 = 100*delays(2)/transmitedPackets(2);
apd1518 = 100*delays(3)/transmitedPackets(3);
